...
Object.assign(ENCOUNTER_PARAM_DEFAULTS, globalThis.TD_ENCOUNTER_TUNING?.encounterDefaults || {});
...
const tuningDefaults = globalThis.TD_ENCOUNTER_TUNING?.defaultParams;
if (tuningDefaults && typeof tuningDefaults === "object") Object.assign(DEFAULT_PARAMS, tuningDefaults);
...
PARAM_GROUPS.unshift([
  ...
  [
    ...
    ["mathPostBossIncrementBossShare", "王後增量BET的後王預算", "比例", 0, .99, .01, "擊殺BOSS後提高的BET增量，有多少RTP預算保留給下一隻BOSS；另一部分自動歸一般波，兩者總和不超過目標RTP。"],
...
const ENCOUNTER_PARAM_GROUPS = [
  ...
  ["V273 BOSS / 每次必加倍率", "boss-group", [
    ...[["Small","小增幅",20],["Medium","中增幅",20],["Large","大增幅",20],["Jackpot","頭獎增幅",50]].flatMap(([key,label,ceiling]) => [
      ...
      [`encounterBoss${key}Max`, `${label}上限`, "+倍率", .01, ceiling, .01, "同一組區間內均勻抽取；不回收已取得的 POT。"],
    ]),
    ["encounterBossDepthGrowth", "每隻後續 BOSS 增幅成長", "比例", 0, 1, .01, "增幅乘以 1 + (BOSS 序號 - 1) × 成長。"],
    ["encounterBossDepthGrowthCap", "後續 BOSS 增幅成長上限", "倍", 1, 5, .05, "限制深追成長倍數；只限制序號成長，不限制頭獎本身。"],
  ]],
...
function cleanParams(input={}) {
  ...
  ["Chest1", "Chest2", "Chest3", "Chest4", "BossChest", "BossSmall", "BossMedium", "BossLarge", "BossJackpot"].forEach(part => {
    const ceiling = part === "BossJackpot" ? 50 : 20;
...
function parameterLogic(key) {
  ...
  if (/^encounter(?:Chest\d|BossChest)/.test(key) || ["encounterRewardScale","encounterPotEntryPower"].includes(key)) return "候選規則：整波 POT = floor(BET × 區間抽籤 × moneyMul × encounterRewardScale ÷ 當下累積倍率 ^ encounterPotEntryPower + 上波小數餘額)，餘額留到下一波";
  if (key.startsWith("encounterBoss")) return "BOSS 增幅 = max(0.1, round(權重抽籤區間值 × min(深度成長, 成長上限), 0.1))";
  ...
  const exact = {
    ...
    mathPostBossIncrementBossShare:"王後增量BET的後王預算 = 增量BET × 入水倍率 × 本值 / (一般波RTP預算+BOSS RTP預算)；增量的一般波預算使用剩餘比例，總期望不變",
...
function renderPoolLedgerLogic() {
  ...
  const rows = [
    ...
    ["2. 王後增量BET拆帳", "擊殺BOSS後玩家繼續並支付提高的BET", "基礎BET沿用一般/BOSS預算比例；BOSS階梯增加的BET改用(目標RTP-後王占比)/後王占比拆帳", "提高BET直接累積下一隻BOSS預算；一般波與BOSS合計期望仍等於目標RTP"],
    ["3. 建立波次合約", "玩家按下繼續並扣BET後", "本波公平預算 = 既有應付 + 本波入水額 × 波次獎勵係數", "一般波採中低幅波段；BOSS才使用高倍尾端"],
    ["4. 依通關率反推彩金", "開波時", "計價率 = 戰鬥預估通關率 × BOSS順位/深度計價倍率；條件彩金 = 公平預算 ÷ 計價率", "計價率 × 條件彩金 = 公平預算"],
    ["5. 戰鬥中升級重定價", "完成3選1後", "新條件彩金 = (舊條件彩金×舊計價率 + 尚未定價入水額) ÷ 新計價率", "沿用相同BOSS順位/深度計價倍率；已顯示POT不可下降"],
    ["6. Reroll獨立入水", "玩家每次升級最多使用一次", "Reroll費用 = 當前BET；Reroll入水額 = 費用 × 獨立抽中倍率", "戰鬥中計入當波；波間先承諾下一波後才可使用"],
    ["7. BOSS倍率", "擊殺BOSS時", "總倍率 = 1 + 各BOSS加成倍率相加", "主要VI來源；首王偏低倍，後王保留高倍尾端"],
    ["8. POT風險延續", "玩家帶著POT繼續BET時", "新公平預算=上一波已公開應付+本波入水；上一波已保留的BOSS來源金額只能原額帶入，不能順帶解鎖其他BOSS子池", "倍率可上升、持平或下降；不會只因總BET增加而被帳本硬性稀釋"],
    ["9. 分帳與深追釋放", "計算波末可領彩金時", "每筆入水先拆為一般子池與BOSS子池；一般波可使用一般子池與上一波已公開的POT，BOSS波才可新增動用BOSS子池", "尚未公開的BOSS預算仍保留；一般波釋放率只作用於未公開舊餘額"],
    ["10. 跨局BOSS預算成熟", "玩家開始下一局時", "成熟額 = 上局剩餘BOSS子池 × 成熟率；只解除用途限制，個人池總available不變", "讓固定早收手玩家的長期RTP也能收斂，不會永久鎖住BOSS預算"],
    ["11. 個人預算帳本", "清波預留與Collect支付時", "實際預留 = min(條件彩金, 累計BET×全局Max Win, 當下可用子池餘額+上一波已保留應付)", "available 永不為負；不與其他玩家互補，也不預支未來投注"],
    ["12. 顯示即實付", "清波與Collect時", "Collect實付 = 畫面公開總得分 = 已預留金額", "波末不可縮POT；不可在Collect時突然補成另一個數字"],
    ["13. 帳本守恆與驗證", "每次入水、預留、支付後", "seed + contributed = available + reserved + paid；Cash Out RTP = Σ實付÷Σ總BET", "守恆誤差必須為0；正式判定看Cash Out RTP，不以帳面配置RTP代替"],
    ["14. 策略公平驗證", "比較不同Build策略時", "每名玩家固定一種策略並持有獨立個人水池；策略RTP = 該策略Σ實付÷Σ該策略BET；配置RTP = 實領RTP + 期末未付責任÷Σ該策略BET", "策略可改變深度、VI與死亡位置，但相同Collect規則的長期配置RTP應接近"],
  ];
...
function logicFormulaRows() {
  return [
    ...
    ["王後增量BET拆帳", "本波BET、基礎BET、已擊殺BOSS、一般/BOSS預算比例、後王預算比例", "bossRaisedStake=max(0,min(本波BET,基礎BET×BOSS階梯)-基礎BET)；增量BOSS預算=bossRaisedStake×入水倍率×後王占比/配置RTP；增量一般預算使用剩餘比例", "一般子池與下一王保留子池", "registerMathStake", "兩個子池合計仍等於該筆入水，不新增RTP；深度波次額外BET不冒充BOSS增量"],
...
function applyToGame() {
  ...
  if (["Small", "Medium", "Large", "Jackpot"].every(part => params[`encounterBoss${part}Weight`] === 0)) {
    ui.status.textContent = "未更新：BOSS 四組增幅權重不可全部為零。";
    return;
  }
...
ui.import.addEventListener("click", () => {
  ...
  try {
    ...
    if(TUNER_CURRENT) {
      ...
      const currentRewardRevision=Number(DEFAULT_PARAMS.encounterRewardRevision)||0;
      const legacyRewards=currentImportNeedsRewardMigration(imported,currentRewardRevision);
      if(legacyRewards) candidate=migrateEncounterRewardParams(candidate);
      const error=validateCurrentDraft(candidate);
      if(error) throw new Error(error);
      params=cleanParams(candidate); build(); ui.status.textContent=legacyRewards?`舊版遭遇戰設定已轉換為 ${currentRewardRevision} 草稿，尚未套用到遊戲。`:"已匯入草稿，尚未套用到遊戲。";
